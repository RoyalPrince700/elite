...
const RequestsTab = ({ filteredRequests, handleStatusUpdate, handleSendInvoice, getStatusColor, getStatusIcon }) => {
  return (
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm text-gray-600">
                  ...
                  <div>
                    <strong>Plan:</strong> {request.planId?.name || 'N/A'}
                  </div>
                  <div>
                    <strong>Cycle:</strong> {request.billingCycle}
                  </div>
                  <div>
                    <strong>Amount:</strong> {request.currency === 'NGN' ? `₦${request.finalPrice.toLocaleString()}` : `$${request.finalPrice}`}
                  </div>
                </div>
